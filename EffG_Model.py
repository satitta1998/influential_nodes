...
def calculate_effective_distance(prob_matrix, shortest_paths_dict, index_to_node_id, node_id_to_index):
    ...
    effective_distance = np.full((num_nodes, num_nodes), np.float32(np.inf),
                                 dtype=np.float32)  # Initialize all distances to infinity
    ...
    for m_id, reachable_nodes in shortest_paths_dict.items():
        ...
        for n_id in reachable_nodes:
            ...
            if (m, n) not in memo:
                ...
                effective_distance[m, n] = find_min_effective_distance(m, n, prob_matrix, set(), memo,
                                                                       shortest_paths_dict, index_to_node_id, depth=0)
            else:
